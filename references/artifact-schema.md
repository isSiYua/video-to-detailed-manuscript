...
`clean-transcript.json` contains section-composed paragraphs with expanded source IDs, timestamps, headings, and spoken-text content. DeepSeek returns chronological start boundaries; the CLI expands both source-to-unit and unit-to-paragraph ranges. The legacy optional `visual_note` field remains readable for artifact compatibility, but new visual publication is frame-scoped and does not aggregate or publish paragraph-level notes.
...
`visual-manifest.json` contains every reviewed frame with timestamp, nearby source IDs, OCR text, optional internal `vision_description`, perceptual hash, quality metrics, `content_kind`, evidence confidence/completeness/density, and the actual `final_height` when a retained screenshot is replaced from the final stream. Each frame has exactly one `publish_mode`: `drop`, `note_only`, `image_only`, or `image_with_note`. `replacement_markdown` belongs only to `note_only`; `display_note` belongs only to `image_with_note` and is limited to one or two short points. The legacy `keep_image` and `extracted_markdown` fields remain synchronized for artifact compatibility. Text-replaceable frames have an empty final path only after complete coverage is proven; visually indispensable or uncertain frames retain a relative asset path.
