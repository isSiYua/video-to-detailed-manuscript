...
Public `douyin.com/video/...`, `iesdouyin.com/share/video/...`, `v.douyin.com/...`, and ordinary share text containing one of those links need no credential. The adapter adapts the Apache-2.0 Social Media Toolkit public-share parser, reads only the single video's public metadata and media address, and does not fetch comments, profile history, private collections, or account data.

The public share page normally exposes no native subtitle track. The pipeline therefore downloads the video once, passes the same local file to the prepared local ASR, derives an at-most-720p visual-analysis copy when needed, and recaptures retained frames from the original local media up to the configured final height. Expiring signed CDN URLs are never stored in durable metadata. If a work is deleted, private, a Douyin image note, or rejected by platform risk control, the adapter reports that limitation instead of requesting a shared Cookie, public parsing service, or unofficial API key.
