...
Social Media Toolkit / social-post-extractor-mcp (Apache-2.0) is the source of the public Douyin share-page `_ROUTER_DATA` routing and field-mapping approach adapted in `scripts/vtm_core/douyin.py`. The adapted implementation removes MCP, GetNote, comments, cloud ASR, browser automation, and artifact code; adds standard-library networking, redirect/CDN validation, response bounds, video-only scope, one-download reuse, and this project's local ASR/visual/manuscript contracts. The adaptation was verified against version 0.3.0 and commit `72718caf3e7bb08a8bf4b990d074a53aec5bd5b9`. Its Apache-2.0 license is included at `licenses/Apache-2.0.txt`. https://github.com/JNHFlow21/social-post-extractor-mcp

The same Social Media Toolkit revision is the source of the Xiaohongshu `window.__INITIAL_STATE__` note-detail mapping adapted in `scripts/vtm_core/xiaohongshu.py`. The adapted version adds current RedNote-domain support, bare/short/share-link normalization, bounded redirect handling, CDN allowlisting, document-order evidence, deterministic text/image pairing, and explicit image-note-only gates. It does not copy or invoke upstream GetNote, browser, comment, ASR, MCP, or account features. The Apache-2.0 license is included at `licenses/Apache-2.0.txt`.

slideextract (BSD-2-Clause) was studied for its policy of retaining the last frame from an unchanged slide region, which tends to capture completed animation steps and annotations. No slideextract C or C++ source code is copied or substantially rewritten; this project independently uses its existing FFmpeg dependency on bounded visual-request windows and does not add OpenCV. https://github.com/szanni/slideextract

slidegeist (MIT) was studied for its conservative practice of sampling about 80% through an ordinary scene. This project independently implements that position only as a local fallback when FFmpeg cannot identify a stable interval; short scenes use their midpoint, dynamic/comparison requests are excluded, and an existing later candidate is never moved backwards. No slidegeist source code is copied or substantially rewritten. https://github.com/itpplasma/slidegeist
